...
public:
	...
	*/
	typename std::list<tEdge> &  vertexEdges( tVertex v );
	...
	*/
	typename std::list<tEdge>   & edges()		{ return m_edges; };
	/*!
	List of the faces of the mesh.
	*/
	typename std::list<tFace>   & faces()		{ return m_faces; };
	/*!
...
protected:
  ...
  /*! list of edges */
  typename std::list<tEdge>                          m_edges;
  /*! list of vertices */
  typename std::list<tVertex>                        m_verts;
  /*! list of faces */
  typename std::list<tFace>							m_faces;
...
CBaseMesh<CVertex,CEdge,CFace,CHalfEdge>::~CBaseMesh()
{
  ...
  for(typename std::list<CFace*>::iterator fiter = m_faces.begin(); fiter != m_faces.end(); fiter ++ )
  {
      ...
      typename std::list<CHalfEdge*> hes;
      do{
        he = halfedgeNext( he );
        hes.push_back( he );
...
inline CEdge * CBaseMesh<CVertex,CEdge,CFace,CHalfEdge>::vertexEdge( tVertex  v0, tVertex  v1 )
{
	...
	typename std::list<CEdge*> & ledges = vertexEdges( pV );
...
//access vertex->edges
template<typename CVertex, typename CEdge, typename CFace, typename CHalfEdge>
inline typename std::list<CEdge*> & CBaseMesh<CVertex,CEdge,CFace,CHalfEdge>::vertexEdges( tVertex  v0 )
{
	return (typename std::list<CEdge*> &)v0->edges();
};
...
*/
	...
	typename std::list<CVertex*> dangling_verts;
	//Label boundary edges
	for(typename std::list<CVertex*>::iterator viter = m_verts.begin();  viter != m_verts.end() ; ++ viter )
	{
		CVertex *     v = *viter;
	 ...
	 for(int i = 0; i < 3; i ++ )
	 {
		...
		for(int i = 0; i < 3; i ++ )
		{
			...
			if( pS == NULL )
			{
				...
				// modified by Jerome
				typename std::list<CEdge*> & ledges0 = (typename std::list<CEdge*> &) v0->edges();
				if (std::find(ledges0.begin(), ledges0.end(), pE) != ledges0.end())
				{
					ledges0.remove(pE);
				}

				typename std::list<CEdge*> & ledges1 = (typename std::list<CEdge*> &) v1->edges();
